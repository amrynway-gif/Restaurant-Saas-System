...
export function MenuFeaturedStoriesBanner({
 ...
 className="relative mt-5 overflow-hidden rounded-2xl border border-[var(--border)] bg-stone-950 shadow-xl shadow-black/10 "
 >
 <div
 ...
 <div
 key={it.id}
 className="h-0.5 flex-1 overflow-hidden rounded-full bg-[var(--bg-surface)]"
 >
 {i < safeIndex ? (
 ...
 className="h-full origin-left bg-[var(--bg-surface)]"
 initial={{ scaleX: 0 }}
 animate={{ scaleX: 1 }}
 transition={{
 ...
 <div className="h-full w-0 bg-[var(--bg-surface)]" />
 )}
 </div>
 ))}
 </div>
 ...
 >
 Zum Verschieben ziehen oder links und rechts auf das Bild tippen – die obere Leiste füllt sich automatisch und wechselt zum nächsten Gericht
 </p>
 ) : null}
 </motion.div>
 );
